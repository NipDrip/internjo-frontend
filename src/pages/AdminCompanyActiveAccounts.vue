...
api.get('http://localhost:3000/active_companies').then((res) => {
  console.log(res)
  active_accounts.value = res.data
})

async function deactivate(i) {
  // console.log(active_accounts.value[i].id)
  await api.put('http://localhost:3000/deactivate_company/' + active_accounts.value[i].id)
